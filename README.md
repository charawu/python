...
关于字符串操作记住基本格式就行以下都直接举例(最简单的此处不举例)
```python
# 索引,以字符串名称[参数]构成
...
```python
len(字符串)          返回字符串长度
str(非字符串)        把非字符串转换成字符串
ord(字符)            返回字符对于的Unicode码
char(Unicode码)     返回Unicode码对于的字符
```
还有很多字符串处理方法,需要时自己查询,有查找,替换等等方法
